...
describe('exponentialBackoffDelay', () => {
  it('returns baseDelay * factor^0 = baseDelay for attempt 0 (no jitter)', () => {
    expect(exponentialBackoffDelay(0, { baseDelay: 300, factor: 2, jitter: false })).toBe(300);
  });

  it('returns baseDelay * factor^1 for attempt 1 (no jitter)', () => {
    expect(exponentialBackoffDelay(1, { baseDelay: 300, factor: 2, jitter: false })).toBe(600);
  });

  it('returns baseDelay * factor^2 for attempt 2 (no jitter)', () => {
    expect(exponentialBackoffDelay(2, { baseDelay: 300, factor: 2, jitter: false })).toBe(1200);
  });

  it('caps result at maxDelay', () => {
    const result = exponentialBackoffDelay(10, { baseDelay: 300, maxDelay: 5000, factor: 2, jitter: false });
    expect(result).toBe(5000);
  });

  it('applies jitter when jitter=true — results should not all be identical', () => {
    const results = Array.from({ length: 20 }, () =>
      exponentialBackoffDelay(2, { baseDelay: 300, factor: 2, maxDelay: 10000, jitter: true })
    );
  ...
  it('jittered value is within [capped/2, capped] range', () => {
    for (let i = 0; i < 20; i++) {
      const result = exponentialBackoffDelay(1, { baseDelay: 300, factor: 2, maxDelay: 10000, jitter: true });
